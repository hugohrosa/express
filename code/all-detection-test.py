...
try:
    ...
    #labels = ['Imparidade','Comparação','Metáfora','Hipérbole','Vulgarismo','Paradoxo']
    for lbl in labels:
        log.write('\n#### ' + lbl + ' ####\n')
        lbl_size = 0
        ...
        lbl_y = 0
        lbl_n = 0
        split_trn = round(lbl_size*0.85)
        split_tst = round(lbl_size*0.15)
        ...
        for row in csv.DictReader(open('DATA/data_all.csv', 'rU') , delimiter = '\t'):
            ...
            if int(row[lbl])>=1 and lbl_y < split_trn:
                lbl_y+=1
                train.append((row['texto'].lower(),1))
            elif int(row[lbl])==0 and lbl_n < split_trn:
                lbl_n+=1
                train.append((row['texto'].lower(),0))
            elif int(row[lbl])>=1 and lbl_y >= split_trn  and lbl_y < split_trn + split_tst:
                lbl_y+=1
                test.append((row['texto'].lower(),1))
            elif int(row[lbl])==0 and lbl_n >= split_trn  and lbl_n < split_trn + split_tst:
                lbl_n+=1
        ...
        log.write('TOTAL SPLIT: ' + str(lbl_y) + ' - ' +str(lbl_n)+'\n')
...
##### DAQUI PARA BAIXO É CNN #########        
    ...
    #     model.fit( train_sequences , train_labels , nb_epoch=5, batch_size=256, show_accuracy=False )
    #     results = model.predict_classes( test_sequences )
    #     #results = model.predict({'input': test_sequences })['output']
    ...
    #     model.fit({'input': train_sequences, 'output': train_labels}, batch_size=256, nb_epoch=5)
    #     #results = model.predict_classes( test_sequences )
    #     results = model.predict({'input': test_sequences })['output']
    #     results = np.round(results)
    #     log.write("Accuracy = " + repr( sklearn.metrics.accuracy_score( test_labels , results )  )+'\n')
    ...
    #     log.write(sklearn.metrics.classification_report( test_labels , results )+'\n')
        ...
        #model.add(Dense(1, activation='sigmoid'))
        #model.compile(loss='binary_crossentropy', optimizer='rmsprop', class_mode='binary')
        #model.fit( train_rep , train_labels , nb_epoch=1, batch_size=256, show_accuracy=False)
    ...
    #       model.fit({'input': train_X_slice, 'output': train_Y_slice}, batch_size=256, nb_epoch=5)
